...
class VisionIssueApp(tk.Tk):
    ...
    def build_deep_learning_tab(self) -> None:
        self.deep_learning_tab.columnconfigure(0, weight=1)
        self.deep_learning_tab.rowconfigure(1, weight=1)
        toolbar = ttk.Frame(self.deep_learning_tab, style="Panel.TFrame", padding=12)
        ...
        body_container = ttk.Frame(self.deep_learning_tab)
        ...
        body_canvas = tk.Canvas(body_container, background="#f4f6f8", highlightthickness=0)
        body_canvas.grid(row=0, column=0, sticky="nsew")
        body_scroll = ttk.Scrollbar(body_container, orient="vertical", command=body_canvas.yview)
        body_scroll.grid(row=0, column=1, sticky="ns")
        body_canvas.configure(yscrollcommand=body_scroll.set)
        content = ttk.Frame(body_canvas)
        content_window = body_canvas.create_window((0, 0), window=content, anchor="nw")
        content.bind("<Configure>", lambda _event: body_canvas.configure(scrollregion=body_canvas.bbox("all")))
        body_canvas.bind("<Configure>", lambda event: body_canvas.itemconfigure(content_window, width=event.width))
        content.columnconfigure(0, weight=3)
        content.columnconfigure(1, weight=2)

        dashboard_panel = ttk.Frame(content, style="Panel.TFrame", padding=12)
        dashboard_panel.grid(row=0, column=0, sticky="nsew", padx=(0, 10), pady=(0, 10))
        dashboard_panel.columnconfigure(0, weight=1)
        dashboard_panel.rowconfigure(1, weight=1)
        ...
        queue_panel.grid(row=0, column=1, sticky="nsew", pady=(0, 10))
        ...
        trained_xscroll = ttk.Scrollbar(queue_panel, orient="horizontal", command=self.dl_trained_tree.xview)
        trained_xscroll.grid(row=2, column=0, sticky="ew")
        self.dl_trained_tree.configure(yscrollcommand=trained_scroll.set, xscrollcommand=trained_xscroll.set)
        self.dl_trained_tree.bind("<<TreeviewSelect>>", lambda _event: self.load_selected_dl_trained_model())

        forms = ttk.Frame(content)
        forms.grid(row=1, column=0, columnspan=2, sticky="nsew")
        ...
        self.bind_mousewheel_recursive(content, body_canvas)
        self.refresh_deep_learning_models()
