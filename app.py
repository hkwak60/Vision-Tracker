...
class VisionIssueApp(tk.Tk):
    ...
    def build_layout(self) -> None:
        ...
        self.open_tab = ttk.Frame(notebook, padding=14)
        self.entry_tab = ttk.Frame(notebook, padding=14)
        self.search_tab = ttk.Frame(notebook, padding=14)
        notebook.add(self.open_tab, text="Open Issues")
        notebook.add(self.entry_tab, text="New / Edit Issue")
        notebook.add(self.search_tab, text="Search & Excel Report")
    ...
    def build_entry_tab(self) -> None:
        ...
        self.issue_time_var = tk.StringVar(value=now_text())
        ...
        self.worker_var = tk.StringVar(value=WORKERS[0])
        ...
        self.add_labeled_entry(panel, "Issue Time", self.issue_time_var, 1, 0)
        ...
        self.add_labeled_combo(panel, "Worker", self.worker_var, WORKERS, 2, 2)
        category_combo = self.add_labeled_combo(panel, "Category", self.category_var, CATEGORIES, 3, 0)
        category_combo.bind("<<ComboboxSelected>>", lambda _event: self.update_subcategories())
        self.subcategory_combo = self.add_labeled_combo(panel, "Subcategory", self.subcategory_var, CATEGORY_MAP[self.category_var.get()], 3, 2)
        self.add_labeled_combo(panel, "Status", self.status_var, STATUS_OPTIONS, 4, 0)
        self.add_labeled_entry(panel, "Resolved Time", self.resolved_time_var, 4, 2)
    ...
    def form_issue(self) -> IssueInput:
        return IssueInput(
            issue_time=self.issue_time_var.get().strip(),
    ...
    def clear_form(self) -> None:
        ...
        self.issue_time_var.set(now_text())
